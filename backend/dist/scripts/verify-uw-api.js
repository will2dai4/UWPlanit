...
async function verifyAPI(termCode = '1249') {
    console.log('╔══════════════════════════════════════════════════════╗');
    ...
    console.log('   ────────────────────────────────────────────────');
    const checks = {
        'API Base URL': env_1.config.uwApi.baseUrl,
        'API Key Configured': env_1.config.uwApi.apiKey ? '✅ Yes' : '❌ No',
        'API Key Length': env_1.config.uwApi.apiKey?.length || 0,
    };
    ...
    console.log('   ────────────────────────────────────────────────');
    console.log(`   Endpoint: ${env_1.config.uwApi.baseUrl}/Courses/${termCode}`);
    console.log(`   Term Code: ${termCode}`);
    console.log('');
    try {
        console.log('   Sending request...');
        const url = `${env_1.config.uwApi.baseUrl}/Courses/${termCode}`;
        const response = await fetch(url, {
            headers: {
                'Accept': 'application/json',
                'X-API-KEY': env_1.config.uwApi.apiKey,
            },
        });
        ...
        if (error instanceof Error && error.stack) {
            logger_1.logger.error({ err: error, stack: error.stack }, 'API verification failed');
        }
        process.exit(1);
    }
}
...
verifyAPI(termCode);
//# sourceMappingURL=verify-uw-api.js.map