    ...
    try {
        ...
        for (const course of sampleCourses) {
            try {
                ...
                const existing = await database_1.db.queryOne('SELECT course_id FROM courses WHERE subject = $1 AND catalog_number = $2', [course.subject, course.catalog_number]);
                // Upsert course
                const query = `
          ...
          RETURNING course_id
        `;
                await database_1.db.queryOne(query, [
                    course.subject,
        ...
        for (const rel of relationships) {
            try {
                const sourceCourse = await database_1.db.queryOne('SELECT course_id FROM courses WHERE subject = $1 AND catalog_number = $2', [rel.source, rel.sourceCat]);
                const targetCourse = await database_1.db.queryOne('SELECT course_id FROM courses WHERE subject = $1 AND catalog_number = $2', [rel.target, rel.targetCat]);
             ...
             VALUES ($1, $2, 'PREREQ')
             ON CONFLICT DO NOTHING`, [sourceCourse.course_id, targetCourse.course_id]);
        ...
        console.error('');
        logger_1.logger.error({ err: error }, 'Sample data population failed');
        process.exit(1);
    }
}
main();
//# sourceMappingURL=populate-with-sample-data.js.map