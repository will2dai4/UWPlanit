...
async function seed() {
    try {
        ...
        await database_1.db.query(`INSERT INTO users (user_id, email, display_name)
       VALUES ($1, $2, $3), ($4, $5, $6)
       ON CONFLICT (user_id) DO NOTHING`, [
            userId1, 'alice@example.com', 'Alice Smith',
            userId2, 'bob@example.com', 'Bob Johnson',
        ]);
        logger_1.logger.info('Sample users created');
        ...
        for (const course of courses) {
            const result = await database_1.db.queryOne(`INSERT INTO courses (subject, catalog_number, title, units, description, terms_offered, faculty)
        ...
        logger_1.logger.info('Sample courses created');
        ...
        for (const rel of relations) {
           ...
           VALUES ($1, $2, $3)
           ON CONFLICT DO NOTHING`, [rel.source, rel.target, rel.rtype]);
            }
        }
        logger_1.logger.info('Sample course relations created');
        // Create sample plan for Alice
        const plan = await database_1.db.queryOne(`INSERT INTO plans (user_id, name)
       VALUES ($1, $2)
       RETURNING plan_id`, [userId1, 'My First Year Plan']);
        if (plan && courseIds['CS135']) {
            // Add CS 135 to the plan
            await database_1.db.query(`INSERT INTO plan_items (plan_id, course_id, term, pos_x, pos_y)
         VALUES ($1, $2, $3, $4, $5)`, [plan.plan_id, courseIds['CS135'], 'F2024', 100, 100]);
            // Add CS 136 to the plan
            if (courseIds['CS136']) {
                await database_1.db.query(`INSERT INTO plan_items (plan_id, course_id, term, pos_x, pos_y)
           VALUES ($1, $2, $3, $4, $5)`, [plan.plan_id, courseIds['CS136'], 'W2025', 300, 100]);
            }
            logger_1.logger.info('Sample plan created');
        }
        logger_1.logger.info('Database seeding completed successfully');
        process.exit(0);
    }
    catch (error) {
        logger_1.logger.error({ err: error }, 'Database seeding failed');
        process.exit(1);
    }
}
seed();
