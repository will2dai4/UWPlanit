 ...
 */
export const parseChecklistText = (text: string): ParsedChecklistItem[] => {
  const lines = text.split('\n').filter(line => line.trim().length > 0);
  const result: ParsedChecklistItem[] = [];
  const stack: { item: ParsedChecklistItem; indent: number }[] = [];
...
export const flattenChecklistItems = (
  ...
  items.forEach((item, index) => {
    ...
    if (item.children && item.children.length > 0) {
      ...
      const childItems = flattenChecklistItems(item.children, planId, tempId);
