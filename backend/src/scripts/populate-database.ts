...
async function main() {
  try {
    ...
    console.log(`   API Base: ${config.uwApi.baseUrl}`);
    ...
    const result = await etlService.runETL(termsToProcess);
    ...
    if (error instanceof Error && error.stack) {
      logger.error({ err: error, stack: error.stack }, 'Database population failed');
    }
